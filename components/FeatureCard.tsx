...
const FeatureCard: React.FC<Omit<Feature, 'pro'>> = ({ icon, title, description }) => {
  return (
    <div className="group relative bg-light-bg/50 border border-slate-700 rounded-xl p-5 sm:p-6 transition-all duration-300 hover:border-primary hover:shadow-2xl hover:shadow-primary/20 hover:-translate-y-2">
      ...
      <div className="relative z-10">
        ...
        <p className="text-medium-text text-sm sm:text-base leading-relaxed">{description}</p>
      </div>
    </div>
  );
};
